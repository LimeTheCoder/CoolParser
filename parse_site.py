...
BASE_URL = 'http://www.posolstva.org.ua'


def process_text(text_lst):
    text_lst = map(lambda x: ' '.join(x.split()), text_lst) # remove two+ tabulation chars
    text_lst = filter(lambda x: len(x) > 2, text_lst) # get words with two and more chars
    return text_lst


def process_images(images):
    return map(lambda img: img if img.startswith("http") else BASE_URL + img, images)
